...
export class FloatingElementsBundle extends GE.ADynamicObject {
	...
	public registerManagedElements(styleID: string): void {
		...
		for (let i = 0; i < elements.length; i++) {
			...
			if (this.elementPositions.has(element)) continue;
		...
		console.log(`FloatingElements registered "${this.elementPositions.size}" elements`);
	}
	...
	public updateElementPosition(element: HTMLElement, originalPosition: CT.Vector2d): void {
		// offseted by horizontal axis
		const time = GE.GameTime.realTimeSinceStartup + originalPosition.x * this.xPositionTimeInfluence;
		...
		let megaCursorInfluence =
			MG.MegaCursor.getDistance(originalPosition) * this.megaCursorInfluenceDistance;
		megaCursorInfluence = Math.max(0, Math.min(1, megaCursorInfluence));

		const offsetX = Math.sin(time * this.motionSpeed) * this.motionAmplitude * megaCursorInfluence;

		const offsetY = Math.cos(time * this.motionSpeed) * this.motionAmplitude * megaCursorInfluence;
...
export class FloatingElement extends GE.ADynamicObject {
	...
	public override onFrameUpdate(): void {
		this.updateElementPosition(this._element, this._elementOriginalPositions);
	}

	private updateElementPosition(element: HTMLElement, originalPosition: CT.Vector2d): void {
		const timeOffset = originalPosition.x * this.xPositionTimeAmplitude * this.xPositionTimeInfluence;
		// offseted by horizontal axis
